...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const Task = values.nome+"%"+values.email+"%"+values.telefone+"%"+values.convenio+"%"+values.carteirinha+"%"+values.medico;
      
    //console.log(Task);
   
    return window.location.replace("https://wa.me/5531993430841?text="+values.nome+"%0A"+values.email+"%0A"+values.convenio+"%0A"+values.carteirinha+"%0A"+values.medico);
            ...
            <div className="w-full  sm:w-1/2 relative z-[1] bg-white shadow-btn rounded-2xl p-8 flex flex-col justify-between">
              <div className="w-full flex flex-row flex-wrap gap-y-4 gap-x-2">
                <div className="w-full md:w-[calc(50%-8px)]" >
                  ...
                  <input onChange={handleChange} type="text" name="nome" className={`capitalize + ${inputStyle}`} placeholder="Nome completo..." />
                </div>
                <div className="w-full md:w-[calc(50%-8px)]" >
                  <label className="block h5 text-black mb-2">Email:</label>
                  <input onChange={handleChange} type="email" name="email" className={inputStyle} placeholder="Email pessoal..." />
                </div>
                <div className="w-full md:w-[calc(50%-8px)]" >
                  <label className="block h5 text-black mb-2">Telefone:</label>
                  <input onChange={handleChange} type="tel" name="telefone" className={inputStyle} placeholder="Número pessoal..." />
                </div>
                <div className="w-full md:w-[calc(50%-8px)]" >
                  <label className="block h5 text-black mb-2">Convênio:</label>
                  <select onChange={handleChange} name="convenio" className={inputStyle}>
                ...
                <div className="w-full md:w-[calc(50%-8px)]" >
                  ...
                  <input onChange={handleChange} type="number" name="carteirinha" className={inputStyle} placeholder="Ref..." />
                </div>
                <div className="w-full md:w-[calc(50%-8px)]" >
                  <label className="block h5 text-black mb-2">Médico:</label>
                  <select onChange={handleChange} name="medico" className={inputStyle}>
              ...
              <div className="self-center">
                <button onClick={handleSubmit} className="flex justify-center items-center gap-2 w-full sm:w-max bg-primary rounded-xl px-8 py-4 font-bold text-white shadow-btn hover:bg-secondary hover:-translate-y-1 transition duration-300">
